...
- Дані: LDAP (`System.DirectoryServices.Protocols`, `ldap://127.0.0.1`), процеси через `System.Diagnostics.Process` з `ArgumentList` (`ProcessRunner` / `SudoRunner`, без CliWrap), файли через `System.IO` у `/srv/data`, SQLite (`Microsoft.Data.Sqlite` + Dapper) для журналу.
...
dotnet run --project src/Scm.Api            # бекенд на http://localhost:5000
cd web && npm install && npm run dev        # Vite на :5173, проксі /api і /healthz → http://localhost:5000
cd web && npm run gen:api    # OpenAPI → src/api/client.ts (потрібен запущений бекенд)
cd web && npm test -- --run  # без --run Vitest лишається в режимі спостереження
...
7. Парсери виводу команд тестуються на фікстурах із `tools/pi-fixtures/`. Нова команда на Pi = нова фікстура в тому ж коміті. Ім'я файлу: `<команда>.<samba-версія>.<txt|json|ldif>`; для команд, що не залежать від Samba (`df`, `ls`), — без версії. Фікстури лише справжні, з Pi; не вигадувати.
...
- Логін учня — `прізвище.ім'я.рік_народження` (`ivanenko.petro.2011`), один на все навчання; > 20 символів — ім'я до ініціала; збіг вирішує вчитель вручну.
- Клас — лише група `uchni-<рік>-<клас>` (`uchni-2025-4a`); поточний = група з найновішим роком; переведення учня між класами немає, щороку весь клас зараховується в новий.
- `submit` — підпапка на учня (`class/<рік>-<клас>/submit/<login>/`), її створює GPO logon-скрипт; «хто не здав» = порожня або відсутня підпапка.
...
- Профілі учнів на ПК — обов'язковий профіль + Folder Redirection + диск `H:` (`docs/deployment.md`, Етапи 8–9); роумінгових профілів немає.
- Назва: School Class Manager. Ліцензія: MIT.

## Стан

- M0 (інфраструктура за `docs/deployment.md`) — виконується вручну; фікстур у `tools/pi-fixtures/` ще немає.
- M1, сесії 1, 2 і 2a: скелет рішення, `/healthz`, SPA з `AppLayout` і «Учні» на даних мокапу, CI; `ProcessRunner`, `SudoRunner`, `SudoAccountCommands`, тести `scm-user`; формат логіна `ivanenko.petro.2011` і класу `2025-4a`, `enroll` замість `move`.
- Наступна — сесія 3 (`LdapDirectory`, читання під `svc-scm`): потрібні доступ до Pi або LDIF-фікстури.
