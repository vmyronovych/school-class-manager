...
| --- | --- |
| Учні: список, створення, CSV-імпорт, скидання пароля, розблокування, вимкнення, переведення в клас, картки логінів PDF | Join ПК у домен, GPO, sysprep, Clonezilla — залишаються в інструкції |
...
| Сервер: знімок/бекап зараз, знімки, логи, стан оновлень | Майстер первинного налаштування Pi (він не може працювати на сервері, якого ще нема — залишається скриптом `scm-provision` з інструкції) |
...
**Обмеження.** Один Pi, один домен, до \~500 облікових записів, 2–3 одночасні користувачі. Pi 4 з 4 GB: Samba + бекенд ≈ 400 MB RAM, запас є.
...
| --- | --- | --- |
| Читання каталогу | LDAP через `ldapi:///` (Unix-сокет Samba) або `ldap://127.0.0.1` | Локально, без TLS-питань; `System.DirectoryServices.Protocols` на Linux працює |
...
| --- | --- |
| `Scm.Core` | Моделі, інтерфейси (`IDirectory`, `IAccountCommands`, `IFileStore`, `IServerStatus`, `IJournal`, `ICardPrinter`), use-case-и; без ASP.NET і без Process |
| `Scm.Infrastructure` | `LdapDirectory`, `SudoAccountCommands` (Process + парсери), `LocalFileStore`, `ProcServerStatus`, `SqliteJournal`, `QuestPdfCardPrinter` |
...
| Процеси | `System.Diagnostics.Process` + `CliWrap` | ручний Process | CliWrap дає таймаути, cancel, потоки stdout/stderr без deadlock |
...
| --- | --- | --- |
| `Student` | `login`, `surname`, `givenName`, `classCode`, `status` (Active / Locked / Disabled / NeverLoggedIn), `lastLogon`, `lastLogonPc`, `homeSizeBytes`, `passwordChangedAt` | LDAP: `sAMAccountName`, `sn`, `givenName`, `memberOf`, `userAccountControl`, `lockoutTime`, `lastLogonTimestamp`, `pwdLastSet`; ПК — з `smbstatus`/журналу; розмір — `du` по `/srv/data/home/<login>` |
| `SchoolClass` | `code` (`4a`), `displayName` (`4-А`), `studentCount` | Група `uchni-<код>` |
...
| `ServerStatus` | `online`, `uptimeSec`, `load1`, `ramUsedMb`, `ramTotalMb`, `tempC`, `dataDisk{used,total,smartOk}`, `backupDisk{…}`, `lastBackup{at,ok,durationSec}`, `lastSnapshot{at,ok}`, `sambaVersion` | `scm-status` JSON |
...
| `GET /api/students?class=4a&q=` | список з кешем 30 с | Teacher |
| `GET /api/students/{login}` | картка + знімки папки | Teacher |
| `POST /api/students` | `{surname, givenName, classCode}` → `{login, password}` | Teacher |
...
| `POST /api/students/{login}/move` | `{classCode}` | Teacher |
...
| --- | --- |
| Створити учня | `sudo scm-user create <login> <pass> --given-name --surname --class 4a` → обгортка викликає `samba-tool user create … --userou=OU=Uchni` і `group addmembers uchni-4a` |
| Скинути пароль | `sudo scm-user setpassword <login> <pass>` |
| Розблокувати / вимкнути / увімкнути | \`sudo scm-user unlock |
| Перевести | `sudo scm-user move <login> 4a 5a` (група мінус/плюс; логін не змінюється — рішення з інструкції: у новому році створюються нові логіни) |
| Імпорт | той самий `create` у циклі, послідовно, з таймаутом 10 с на рядок; звіт по кожному |
| Роздатки | запис у `/srv/data/class/4a/handouts/`, `chown` на uid вчителя-власника, права 644 |
| Здані роботи | список `/srv/data/class/4a/submit/<login>/`; власник = назва підпапки; «не здав» = учні класу з порожньою або відсутньою підпапкою |
...
| Учні `/students` | `Radio.Group` (класи), `Input.Search`, `Table` з `rowSelection`, сортуванням і фільтром статусу, `Tag`, `Pagination`; права `Card` з `Descriptions` + кнопки + список знімків | `/students`, `/students/{login}` |
| Файли `/files` | `Tabs` з `Badge`, `Upload.Dragger`, `Table` ×2, `Progress`, `Tag` для «не здали» | `/classes/{code}/handouts`, `/submissions` |
...
- Таблиця з колонками з мокапу; статус — `Tag` (`success`/`warning`/`default`/`error`); клік на прізвище — картка праворуч; `rowSelection` вмикає панель масових дій (скинути паролі, вимкнути, перевести).
- Картка: `Descriptions` (логін, клас, останній вхід і ПК, папка, пароль змінено), дії, знімки папки за 30 днів із «Відновити…» → `Modal` з деревом файлів знімка.
- «+ Учень» — `Modal` з `Form`: клас, прізвище, ім'я; логін і пароль генеруються на сервері й показуються в результаті з кнопкою «Картка PDF».
- «Імпорт CSV» — `Steps`: файл → превʼю (`Table` з підсвіткою помилок: дублікат, невідомий клас, порожнє прізвище) → створення з `Progress` через SSE → звіт → «Картки для створених».
...
- Ярлик «Роздатки» на Робочому столі учня ставить GPO Preferences (Shortcut → `\\dc1\class\%ClassCode%\handouts`) — додати в Етап 8 інструкції; застосунок цим не керує.
...
- Вхід: логін + пароль домену → LDAP simple bind до `ldapi:///` (або `ldap://127.0.0.1`, без TLS, бо не виходить за межі Pi). Невдалий bind = 401; 5 невдач за 10 хв з однієї IP = 429 на 10 хв (в AD теж спрацює lockout — не плутати з блокуванням учня).
- Після bind — пошук `memberOf`: `Domain Admins` → Admin, `vchyteli` → Teacher, інакше 403 «доступ лише для вчителів». Учнівські облікові записи ніколи не проходять.
...
2. Корінь роздається всім ПК через GPO: Computer Configuration → Policies → Windows Settings → Security Settings → Public Key Policies → Trusted Root Certification Authorities. Додати в Етап 8 інструкції.
...
- systemd-юніт `scm.service`: `User=scm`, `Group=scm`, `AmbientCapabilities=CAP_DAC_OVERRIDE CAP_CHOWN CAP_FOWNER` (доступ до файлів учнів без root), `NoNewPrivileges=false` (потрібен sudo), `ProtectSystem=strict`, `ReadWritePaths=/srv/data /var/lib/scm`, `PrivateTmp=yes`.
- `/etc/sudoers.d/scm` — лише обгортки, без wildcard-ів:

  ```
  scm ALL=(root) NOPASSWD: /usr/local/sbin/scm-user, /usr/local/sbin/scm-status, \
    /usr/local/sbin/snap.sh, /usr/local/sbin/backup.sh, /usr/local/sbin/scm-logs
  Defaults:scm !requiretty
  ```
- `scm-user` (bash, \~60 рядків): приймає лише `create|setpassword|unlock|enable|disable|move`; валідує логін regex-ом `^[0-9]{1,2}[a-z]\.[a-z]+(\.[a-z]+)?$`; відмовляє, якщо ціль у `Domain Admins`; ціль у `vchyteli` — лише з прапорцем `--teacher`, який бекенд передає тільки для ролі Admin; інакше ціль має бути в `OU=Uchni`; пароль читає зі stdin, не з аргументів (щоб не світився в `ps`); пише в `journald`.
- `scm-logs`: `journalctl -u <юніт з білого списку> -n 200 --no-pager`.
...
- Компрометація сервісу `scm` = змога міняти паролі учнів, але не адмінів і не вчителів (обгортка), і не читати базу AD (немає прав на `/var/lib/samba/private`).
...
│  │  ├─ Process/                    # SudoRunner (CliWrap, ArgumentList), парсери smbstatus/scm-status/df
...
│  ├─ scm-update.service, scm-update # завантажити реліз, розпакувати, перезапустити
│  ├─ sudoers.d/scm
│  ├─ sbin/scm-user, scm-status, scm-logs
│  ├─ nftables/scm.nft
...
├─ docs/                             # plan.md, deployment.md, teacher-guide.md
└─ .github/workflows/ci.yml          # dotnet test + vitest + vite build + publish linux-arm64 + release tar.gz
```
...
public interface IAccountCommands
{
    ...
    Task<Result> MoveAsync(Login login, ClassCode from, ClassCode to, CancellationToken ct);
}
...
public sealed record Login(string Value);          // валідований у конструкторі
public sealed record ClassCode(string Value);      // "4a"
...
- Команди: `dotnet build`, `dotnet test`, `cd web && npm run dev` (проксі `/api` на `https://dc1…` або локальний бекенд), `npm run gen:api` (OpenAPI → TS).
...
- Як додати фікстуру: команда на Pi → `tools/pi-fixtures/<команда>.<samba-версія>.txt` → тест парсера.
...
5. `appsettings.Production.json` у `/etc/scm/`: realm, base DN, OU, групи, шляхи, PFX, пароль PFX (або порожній).
...
Усе це робить `install.sh` idempotent-но; п.3 і 5 запитують значення інтерактивно.
...
`DOTNET_gcServer=0` — workstation GC, інакше .NET на 4 ядрах відкусить зайві 200 MB. `MemoryMax` — запобіжник, щоб Samba не постраждала.
...
| 3 (шари) | `submit\<login>\` — підпапка на учня: ACL кореня `submit` — `vchyteli` повний, `uchni-<клас>` лише перегляд списку; підпапку створює GPO logon-скрипт (`mkdir` + власник учень); учень RW лише у своїй |
| 4 (бекапи) | `backup.sh` додатково копіює `/var/lib/scm/scm.db` і `/etc/scm/` |
| 8 (GPO) | + Trusted Root: корінь «SCHOOL Class CA»; + Preferences → Shortcut «Роздатки» на Робочий стіл → `\\dc1\class\<клас>\handouts` (по групах `uchni-<клас>` через Item-level targeting); + закладка `https://dc1.ad.school.lan` у Chrome для групи `vchyteli` |
| Експлуатація | Таблиця операцій: усе, крім оновлення ОС, тепер робиться з браузера; SSH — резерв |
...
| --- | --- | --- | --- |
| **M0 · Інфраструктура** | Pi з Samba AD, шари, знімки; ≥1 ПК у домені з GPO; 5 тестових учнів; `scm` + sudoers + обгортки; фікстури виводів | Етапи 1–8 інструкції; `pi/install.sh` частково; зняти виводи `samba-tool user show`, `smbstatus -b`, `df`, `ls .snapshots` у `tools/pi-fixtures` | 3–4 дні (не вечори) |
...
| **M2 · Масові операції** | Створення учня, CSV-імпорт з превʼю і прогресом, картки класу, вимкнення/увімкнення, переведення, множинний вибір | `LoginGenerator`, `PasswordGenerator`, `CsvStudentParser` + валідація; `ImportStudents` як job з SSE; `ImportWizard` (`Steps`); масові дії в `Table.rowSelection`; `PasswordSessionCache` | 1,5 тижня |
| **M3 · Файли** | Роздатки з `Dragger`, здані роботи + «хто не здав» + ZIP + архів, папки учнів (перегляд/завантаження), відновлення зі знімка кнопкою | `LocalFileStore`, `PathGuard`, `OwnerResolver`; стрімінг upload/download; `Files` сторінка з трьома вкладками; `Drawer` з `Tree`; GPO-ярлик «Роздатки» в інструкцію | 1,5 тижня |
...
3. Сесія 3: `LdapDirectory.GetStudentsAsync` проти Pi (мапінг `userAccountControl`, `lockoutTime`, `pwdLastSet`); фікстура LDIF; `GET /api/students` з OpenAPI → `npm run gen:api` → таблиця з реальними даними.
...
| --- | --- | --- | --- |
| Юніт `Core` | `LoginGenerator` (транслітерація КМУ №55, колізії → `4a.ivanenko.p`), `PasswordGenerator` (довжина, без `l/1/O/0`), `CsvStudentParser` (BOM, `;`, лапки, кирилиця, дублікати), value-об'єкти (`Login`, `StorePath` відкидають `..`, `\`, пробіли), use-case-и з фейковими портами (журнал пишеться, заборонені цілі відхиляються) | xUnit + FluentAssertions | кожен коміт |
...
- [ ] Бачить учнів свого класу за 3 с; пошук за прізвищем працює
...
| --- | --- |
| Структура `submit` | Підпапка на учня: `class\<клас>\submit\<login>\`; створює GPO logon-скрипт при першому вході; власник = назва папки. Етап 3 інструкції змінюється |
